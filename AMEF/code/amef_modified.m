function [R, W, I] = amef_modifed(I_hazy, clip_range)
...
W = ones(r,c,N).*gradient(I);
% W = ones(r,c,N).*contrast(I);
...
function C = gradient(I)
    gx = [1 0 -1; 2 0 -2; 1 0 -1]; % gradient filter; emphasized prewitt
    gy = [1 2 1; 0 0 0; -1 -2 -1];
    N = size(I,4);
    C = zeros(size(I,1),size(I,2),N);
    for i = 1:N
        mono = rgb2gray(I(:,:,:,i));
        Ix = abs(imfilter(mono,gx,'replicate')) + 1e-30;
        Iy = abs(imfilter(mono,gy,'replicate'));
        C(:,:,i) = sqrt(Ix.^2 + Iy.^2) .* abs(atan(Iy./Ix)) + mono; %TODO: Implement direction
    end
end
