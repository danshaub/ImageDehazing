% startVal, endVal, and numVals are for choosing how many under exposure
% levels are used and what the values for gamma should be
function [R, W, I] = amef(I_hazy, clip_range, startVal, endVal, numVals)
    I_hazy = im2double(I_hazy);
    I = zeros(size(I_hazy,1),size(I_hazy,2),size(I_hazy,3),6);

    range = linspace(startVal, endVal, numVals);
    ...
    for c=1:3
        I(:,:,c,numVals) = adapthisteq(I_hazy(:,:,c),'clipLimit',clip_range);
    end
